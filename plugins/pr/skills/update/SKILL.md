...
> The label applied to mark a PR as updated is `pr-updated` by default;
> deployments may override it with the `CODEMATE_PR_UPDATED_LABEL` environment
> variable (see `docs/plugin-contracts.md`). Use the effective value so the
> workspace monitor recognizes the PR as ready for review.
