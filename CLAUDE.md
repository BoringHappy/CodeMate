...
- `--pure` - Run the pure image with the local CodeMate home and current directory mounted, starting zsh instead of running setup (no target, GitHub token, or host `git`/`gh` required)
